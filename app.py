...
with st.form("intake_form"):
    ...
    with col1:
        ...
        sqft = st.number_input("Building Square Footage", min_value=0, value=0)
    with col2:
        start_date = st.date_input("Acquisition Date")
        end_date = st.date_input("Target Closing Date")
    with col3:
        gross_sale = st.number_input("Gross Sale Price ($)", min_value=0.0, value=0.0)

    st.header("2. Deal Economics & Deductions")
    col4, col5, col6, col7 = st.columns(4)
    with col4:
        loan_payoff = st.number_input("Loan Payoff Amount ($)", value=0.0)
    with col5:
        commissions = st.number_input("Broker Commissions ($)", value=0.0)
    with col6:
        closing_costs = st.number_input("Other Closing Costs ($)", value=0.0)
    with col7:
        llc_cash = st.number_input("LLC Cash on Hand ($)", value=0.0)
    ...
    # Generic Default data for Cap Table
    default_investors = pd.DataFrame({
        "Investor": ["Investor 1", "Investor 2", "Investor 3", "Investor 4"],
        "Initial Investment": [0.0, 0.0, 0.0, 0.0],
        "% Ownership": [0.0, 0.0, 0.0, 0.0],
        "Cash Distributions (Past)": [0.0, 0.0, 0.0, 0.0]
    })
    ...
    with col9:
        real_world_context = st.text_area("Real-World Context", value="", height=150, placeholder="Explain the strategic reasoning for this sale here...")
...
if submitted:
    ...
    # Safe division for Multiples and Cash-on-Cash
    investor_df["Total Multiple"] = np.where(investor_df["Initial Investment"] > 0, investor_df["Total Return"] / investor_df["Initial Investment"], 0)
    investor_df["Cash-on-Cash"] = np.where(investor_df["Initial Investment"] > 0, investor_df["Total Return"] / investor_df["Initial Investment"], 0)
    ...
    * **Total Return per $1,000 Invested:** {format_currency(true_total_multiple * 1000)}
    ...
    if not edited_scenarios.empty:
        ...
        scen_df["$/per square foot"] = np.where(sqft > 0, scen_df["Sale Price"] / sqft, 0)
        ...
        scen_df["Multiple"] = np.where(total_initial_capital > 0, scen_df["Total distribution"] / total_initial_capital, 0)
        scen_df["Annual Return"] = np.where((hold_years > 0) & (scen_df["Multiple"] > 0), (scen_df["Multiple"] ** (1 / hold_years)) - 1, 0)
